<template>
  <div class="background" v-if="selectedProduct" :style="{ backgroundImage: `url(${selectedProduct.image})` }"></div>
        ...
        <div class="product-list">
          <div v-for="product in products" :key="product.id" class="product-card"
            ...
            <div class="product-overlay"
              :style="{ background: `linear-gradient(45deg, ${product.visual_primary_color}AA, ${product.visual_secondary_color}AA)` }">
            </div>
...
export default {
  ...
  setup() {
    ...
    console.log(products); // Computed list of products from Vuex store
    ...
    const logout = () => {
      // Clear user session or authentication state from Vuex or local storage
      store.commit('auth/logout'); // Assuming you have a logout action in your Vuex store
    ...
    const progressBarStyle = computed(() => {
      const primary = selectedProduct?.value?.visual_primary_color || "#00ccff";
      const secondary = selectedProduct?.value?.visual_secondary_color || "#0066ff";
